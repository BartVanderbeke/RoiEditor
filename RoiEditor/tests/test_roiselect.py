...
def test_roiselect():
    ...
    bench2 = Workbench(original_file, label_file, roi_file, nuke_roi_file, key_to_label_map)
    window2=bench2.build()
    select_outer_rois_vdb3(bench2.cell_rm, step = 1)
    window2.draw_image()

    bench3 = Workbench(original_file, label_file, roi_file, nuke_roi_file, key_to_label_map)
    window3=bench3.build()
    select_outer_rois_vdb4(bench3.cell_rm, step = 1)
    window3.draw_image()

    bench5 = Workbench(original_file, label_file, roi_file, nuke_roi_file, key_to_label_map)
