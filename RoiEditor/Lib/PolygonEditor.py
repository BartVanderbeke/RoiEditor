...
from Roi import Roi   # jouw bestaande Roi class
...
class VertexHandle(QGraphicsEllipseItem):
    ...
    def __init__(self, x, y, radius, editor, index):
        ...
        self.setPen(QPen(Qt.GlobalColor.yellow, 0.25))
        self.setFlags(
...
class PolygonEditor(QObject):
    ...
    def __init__(self, qimage, window_width, roi):
        ...
        self.view = TightFitGraphicsView(self.scene)
        ...
        self.polygon_pen = QPen(Qt.GlobalColor.green, 0.25)
    ...
    def eventFilter(self, obj, event):
        if event.type() == event.Type.KeyPress:
            ...
            if event.key() == Qt.Key.Key_Escape:
                print("Cancelled -> Roi unchanged")
                self.cleanup()
                return True
            elif event.key() in (Qt.Key.Key_Return, Qt.Key.Key_Enter):
                print("Accepted -> Roi updated")
